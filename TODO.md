...
A fresh session should read this file first, then `README.md`, then `CHANGES.md`. Run `composer test` and `composer stan`; setup is in the README's Tests section. State as of September 21, 2026: 621 tests passing in default and random order, PHPStan clean at level 6 with a baseline of 72, down from 224. All 24 classes are namespaced under `Mai\Locations\` in `inc/classes/`, with every old class name kept working through `inc/aliases.php`. Every bug on the list below is fixed, the Publish checkbox is built and driven end to end in a real browser, and all 22 static caches now live in `Mai\Locations\Cache`. `develop` holds work that has never been pushed. Next: the changelog and the release.
...
- **Static caches were the biggest obstacle, and are gone.** Labels, base, options, field lists, post type and taxonomy lists, query defaults, "is filtered", the WooCommerce tabs and user locations all sat in `static` variables for the whole request. A filter added after boot did nothing, so those paths could not be tested in-process at all. All 22 moved into `Mai\Locations\Cache` on September 21, 2026, a plain keyed store with `has`, `get`, `set`, `forget`, `forget_prefixed` and `flush`. `tests/TestCase.php` flushes it before every test, so no public signature changed and one test's saved option can no longer outlive the database rollback. `mailocations_get_options( true )` and `mailocation_get_user_locations( $post_type, true )` keep their `$reset` arguments, since other sites may call them.
- **`Registration/ScenarioRunner.php` boots WordPress in a child process** for things that genuinely happen at registration time: post type and taxonomy registration, rewrite rules, the API key read at boot. Some of its scenarios may now be plain in-process tests, since a filter plus a flush does the job. Not looked at yet.
...
- [x] Move classes one at a time behind the tests, keeping public names working until the release. Make the static caches resettable as each file moves. The classes were done September 15, 2026, the caches September 21, 2026.
