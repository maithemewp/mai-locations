...
A fresh session should read this file first, then `README.md`, then `CHANGES.md`. Run `composer test` and `composer stan`; setup is in the README's Tests section. State as of September 21, 2026: 613 tests passing in default and random order, PHPStan clean at level 6 with a baseline of 72, down from 224. All 24 classes are namespaced under `Mai\Locations\` in `inc/classes/`, with every old class name kept working through `inc/aliases.php`. Every bug on the list below is fixed, and the Publish checkbox is built and driven end to end in a real browser. `develop` holds work that has never been pushed. Next: the remaining static caches become resettable, 20 of the 22 in `inc/`, then the changelog and the release.
