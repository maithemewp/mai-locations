...
/**
 * The clear button is a core/button variation, not an ACF block.
 */
final class FilterClearBlockTest extends TestCase {

	private \Mai_Locations_Filter_Clear_Block $block;

	public function set_up(): void {
		parent::set_up();
		$this->block = new \Mai_Locations_Filter_Clear_Block();
	}
	...
	public function test_core_button_gets_the_boolean_attribute(): void {
		...
		$this->assertSame( [ 'type' => 'boolean' ], $attributes['maiLocationsFilterClear'] );
	}

	public function test_add_block_attribute_ignores_other_blocks(): void {
		$this->assertSame( [ 'attributes' => [] ], $this->block->add_block_attribute( [ 'attributes' => [] ], 'core/paragraph' ) );
		$this->assertSame( [ 'attributes' => [ 'maiLocationsFilterClear' => [ 'type' => 'boolean' ] ] ], $this->block->add_block_attribute( [ 'attributes' => [] ], 'core/button' ) );
	}
	...
	public function test_add_block_variation_receives_a_block_type_object_from_core(): void {
		...
		$this->assertSame(
			[
				[
					'title'      => 'Mai Locations Filter Clear',
					'name'       => 'mailocations-filter-clear',
					'isActive'   => [ 'maiLocationsFilterClear' ],
					'attributes' => [
						'maiLocationsFilterClear' => true,
						'metadata'                => [ 'bindings' => [ 'url' => [ 'source' => 'mai/locations', 'args' => [ 'key' => 'filterClear' ] ] ] ],
					],
				],
			],
	...
	public function test_hides_the_button_when_no_filters_are_active(): void {
		// mailocations_is_filtered_locations() caches its first answer for the process, and $_GET is empty here.
		$this->assertSame( '', do_blocks( '<!-- wp:button {"maiLocationsFilterClear":true} --><div class="wp-block-button"><a class="wp-block-button__link wp-element-button">Clear</a></div><!-- /wp:button -->' ) );
	}
	...
	public function test_leaves_other_buttons_alone(): void {
		...
		$this->assertSame( $markup, $this->block->render_block_variation( $markup, [ 'attrs' => [ 'maiLocationsFilterClear' => false ] ], null ) );
	}
}
